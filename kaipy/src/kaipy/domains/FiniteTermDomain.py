        ...
        for x in constraints_all:
            match x:
                case Kore.Equals(os, s, Kore.EVar(n1, s1), right):
                    if n1 in renaming_2.keys():
                        constraints.append(Kore.Equals(os, s, Kore.EVar(renaming_2[n1], s1), KoreUtils.rename_vars(renaming_back, right)))
                        continue
            match x:
                case Kore.Equals(os, s, left, Kore.EVar(n2, s2)):
                    if n2 in renaming_2.keys():
                        constraints.append(Kore.Equals(os, s, Kore.EVar(renaming_2[n2], s2), KoreUtils.rename_vars(renaming_back, left)))
        _LOGGER.warning(f"Constraints: {[x.text for x in constraints]}")
